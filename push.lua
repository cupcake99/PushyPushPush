...
Push.control = {
    ...
    [79]  = { name="mstr_volume",  cc=79,  note=8,   value=nil,                   hasCC=true,  hasNote=true,  hasLED=false },
...
function Push:update ()
    if self._state.dirty then
        ...
        dummy = string
            for j = 1, 8 do
                if self._state.current.display.line[i].zone[j] ~= "" then
                    string = self._midi.formatLine(string, self._state.current.display.line[i].zone[j], i, j)
                end
